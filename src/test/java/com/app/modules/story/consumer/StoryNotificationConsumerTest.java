...
class StoryNotificationConsumerTest {
    ...
    @Test
    void consume_missingStoryId_routesToDlqAndAcks() throws Exception {
        Message message =
                message(
        ...
        verify(deadLetterPublisher)
                .publish(
                        eq(message),
                        eq(RabbitMqTopologyConfig.STORY_NOTIFICATION_DEAD_LETTER_ROUTING_KEY),
                        any());
        verify(channel).basicAck(1L, false);
        verify(notificationService, never()).create(any(), any(), any(), any(), any());
    }
    ...
    @Test
    void consume_missingOwnerId_routesToDlqAndAcks() throws Exception {
        Message message =
                message(
                        new DomainEventEnvelope(
                                EVENT_ID,
