...
public final class KeysetPage {
     ...
     */
    public static <E> Result<E> of(List<E> rows, int limit, Function<E, Cursor> cursorOf) {
        boolean hasNextPage = rows.size() > limit;
        List<E> content = hasNextPage ? rows.subList(0, limit) : rows;
        String startCursor =
                content.isEmpty() ? null : CursorCodec.encode(cursorOf.apply(content.get(0)));
        String endCursor =
                content.isEmpty()
                        ? null
                        : CursorCodec.encode(cursorOf.apply(content.get(content.size() - 1)));
