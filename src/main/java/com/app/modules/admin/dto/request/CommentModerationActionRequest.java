...
@Schema(description = "Payload for removing or restoring a comment")
public record CommentModerationActionRequest(
        @Schema(
                        description = "Requested comment moderation action",
                        example = "remove_comment",
                        requiredMode = Schema.RequiredMode.REQUIRED)
                @NotNull
                AdminActionType actionType,
        @Schema(
                        description = "Audit reason for the moderation action",
                        example = "Comment contains targeted harassment",
                        requiredMode = Schema.RequiredMode.REQUIRED)
                @NotBlank
                @Size(max = 2000)
                String reason,
