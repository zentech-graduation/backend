...
/**
 * Persistent record of a single email-verification challenge.
 *
 * <p>Stores only the SHA-256 hash of the raw token issued to the user. {@code expiresAt} bounds the
 * validity window; {@code usedAt} marks single-use consumption. Rows are owned by exactly one user
 * and cascade-deleted with that user.
 */
@Entity
