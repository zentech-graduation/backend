...
@Repository
public interface CommentIdempotencyRepository extends JpaRepository<CommentWriteIdempotency, UUID> {

    Optional<CommentWriteIdempotency> findByUserIdAndIdempotencyKey(
    ...
    @Query(
            value =
                    "INSERT INTO comment_write_idempotency (user_id, idempotency_key, request_hash) "
    ...
    @Modifying
    @Query(
            "UPDATE CommentWriteIdempotency e SET e.responseBody = :body "
     ...
     */
    @Modifying
    @Query("DELETE FROM CommentWriteIdempotency e WHERE e.createdAt < :cutoff")
