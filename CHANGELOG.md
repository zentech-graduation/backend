...
### Security
- Development seed data is no longer applied in production: Flyway now scans `classpath:db/migration` only by default, the feed seed moved to a dev-profile-only `classpath:db/dev-seed` location, and an `app.environment` session guard skips the seed DML on any production-tagged database.
