...
### Security
- Removed the email address from JWT access-token claims so account email is no longer readable from the unencrypted token payload; the user (and email, when needed) is resolved server-side from the subject claim.
