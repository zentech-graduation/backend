...
| Table | Owner | Notes |
|-------|-------|-------|
| `support_tickets` | support | One request and the single staff response to it (V92). |
| `support_category_configs` | support | Display metadata for the category enum (V92). |
...
1. `uq_support_tickets_one_open_per_user` (V95), a partial unique index over `user_id` where the status is `open`, `in_progress` or `escalated`. This is what actually holds under a concurrent double submit.
...
Every staff decision writes an `admin_actions` row through `AdminActionRecorder`: `respond_support_ticket`, `reject_support_ticket`, `escalate_support_ticket` (V93, with their config rows in V94).
...
In-product: a `support_ticket_update` notification (V93, config row in V94).
